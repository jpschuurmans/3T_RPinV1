...
% KP_SF_Filter( im, opt ) NEEDS A SQUARE IMAGE!!!
% spatial frequency filter grayscale images
% opt is a struct that contains:
...
elseif strcmp(opt.type, 'gauss')
    ...
    for i = 1:2*nx-1
        for j =1:2*ny-1
            ...
            % Use Gaussian filter.
            filter1(i,j) = exp(-dist^2/(2*opt.cutoff(2)^2));
            filter2(i,j) = exp(-dist^2/(2*opt.cutoff(1)^2));
...
else error('You need to set opt.whichfilter to o, 1 or 2');
end
