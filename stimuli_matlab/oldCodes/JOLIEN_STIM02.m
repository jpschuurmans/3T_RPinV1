...
load([basefolder 'JOLIEN_STIM.mat'],'LC','*back', 'imset','nim','nblockspercondition')
...
outputmat = 'JOLIEN_STIM02.mat';

%% Measure L and C, index face/back pixels
clear paddims_back
theim=1;
rawXY = [size(imset.raw_back{theim},1), size(imset.raw_back{theim},2)];
paddims_back = max(rawXY) - min(rawXY); % make a square image of same dimensions as background
...
ncontinua = nblockspercondition; % same background noise background across conditions, one different for each block of a given condition
nimpercontinua = 2; %  need a start and end points for each interpolation, so 2 starting images per interpolation continua
nscrBack = ncontinua*nimpercontinua;
...
for theframe=1:nscrBack
    ...
    angleV= reshape((phasim{theframe}), 1, prod(size(phasim{theframe})));
    angle_scr= angleV(randperm(length(angleV)));
    rphasim = reshape(angle_scr, size(daimage));
    randspect = amplim{theframe}.*exp(i*rphasim);
    ...
    [mean2(realrandspect) std2(realrandspect)] % check normalization worked
    imset.Back_scr{theframe} = realrandspect;
end
imshow(imset.Back_scr{theframe}); %pause (0.1)

%% background frames, interpolate
thesteps = linspace(0,1,framesperblock);
clear whatim; whatim = 1:2:nscrBack;
for thecon = 1:ncontinua
    n1 = whatim(thecon);
    n2 = n1 + 1;
    sequence = minPhaseInterp( imset.Back_scr{n1}, imset.Back_scr{n2},thesteps);
    Background.sequences{thecon} = sequence;
end

for thecon = 1
    for theframe = 1:size(Background.sequences{thecon} ,3)
        imshow(squeeze(Background.sequences{thecon}(:,:,theframe))); hold on; pause (0.1666)
    end
end
